...
with st.sidebar.expander("General", expanded=True):
    ...
    # Hometown filter: whether fighter's hometown matches event country
    hometown_options = ["All", "Yes (home country)", "No (away)"]
    hometown = st.selectbox("Hometown vs Event Country", hometown_options, key="filter_hometown") if 'HometownFighter' in data.columns and 'EventCountry' in data.columns else "All"
...
# Overwrite data for the rest of the app
data = filtered
# For matchup display, keep both rows from original_data for surviving FightIDs
...
# Overall and recent win rates for shrinkage
