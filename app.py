...
# =========================================================================
# 3D LR WIN/LOSS PREDICTION
# =========================================================================
st.header("3D LR Win/Loss Prediction")
st.markdown("Select three numerical variables. A Logistic Regression model is fitted. Log‑loss, Brier score, win probability, and dataset‑wide win rates are shown.")
...
# =========================================================================
# 3D WEIGHTED KNN WIN/LOSS PREDICTION
# =========================================================================
st.header("3D Weighted KNN Win/Loss Prediction")
st.markdown("Select three numerical variables. A weighted KNN model is fitted. Log‑loss, Brier score, win probability, and dataset‑wide win rates are shown.")
...
# ---------- 2. Categorical Feature Importance ----------
...
# =========================================================================
# 3D SCATTER + COMBINATION BUILDER (LOGISTIC REGRESSION)
# =========================================================================
...
if len(three_d_features) >= 3:
    col1, col2, col3 = st.columns(3)
    with col1:
        x_lr_combo = st.selectbox("X", three_d_features, key="lr_combo_x")
    with col2:
        y_lr_combo = st.selectbox("Y", three_d_features, key="lr_combo_y")
    with col3:
        z_lr_combo = st.selectbox("Z", three_d_features, key="lr_combo_z")

    if x_lr_combo and y_lr_combo and z_lr_combo:
        plot_data = data[[x_lr_combo, y_lr_combo, z_lr_combo, 'DetailedResult', 'Fight']].dropna()
        if len(plot_data) < 10:
            st.warning("Not enough data for 3D plot.")
        else:
            fig = px.scatter_3d(
                plot_data,
                x=x_lr_combo, y=y_lr_combo, z=z_lr_combo,
                color='DetailedResult',
                color_discrete_map=color_map,
                hover_data=['Fight'],
    ...
    if len(candidates_lr) >= 3:
        if st.button("Compute LR 3‑Var Combos", key="lr_combo_btn"):
            with st.spinner("Testing 3‑variable LR combos…"):
                ...
                combo_list = []
                for combo in itertools.combinations(candidates_lr, 3):
                    ...
                    try:
                        ...
                        combo_list.append({'Variables': ', '.join(combo), 'Brier': bs})
                    except:
                        pass
                if combo_list:
...
# =========================================================================
# 3D SCATTER + COMBINATION BUILDER (WEIGHTED KNN)
# =========================================================================
...
if len(three_d_features) >= 3:
    col1_knn, col2_knn, col3_knn = st.columns(3)
    with col1_knn:
        x_knn_combo = st.selectbox("X", three_d_features, key="knn_combo_x")
    with col2_knn:
        y_knn_combo = st.selectbox("Y", three_d_features, key="knn_combo_y")
    with col3_knn:
        z_knn_combo = st.selectbox("Z", three_d_features, key="knn_combo_z")

    if x_knn_combo and y_knn_combo and z_knn_combo:
        plot_data_knn = data[[x_knn_combo, y_knn_combo, z_knn_combo, 'DetailedResult', 'Fight']].dropna()
        if len(plot_data_knn) < 10:
            st.warning("Not enough data for 3D plot.")
        else:
            fig_knn = px.scatter_3d(
                plot_data_knn,
                x=x_knn_combo, y=y_knn_combo, z=z_knn_combo,
                color='DetailedResult',
                color_discrete_map=color_map,
                hover_data=['Fight'],
    ...
    # --- KNN combination builder (3‑variable, with K slider) ---
    ...
    if len(candidates_knn) >= 3:
        if st.button("Compute KNN 3‑Var Combos", key="knn_combo_btn"):
            with st.spinner("Testing 3‑variable Weighted KNN combos…"):
                hist = data[data['Win?'].isin(['Yes','No'])].copy()
                hist['WinNum'] = (hist['Win?'] == 'Yes').astype(int)
                ...
                for combo in itertools.combinations(candidates_knn, 3):
                    ...
                    try:
                        ...
                        y_prob = cross_val_predict(knn, X_scaled, y, cv=5, method='predict_proba')[:, 1]
